...
func TestFailInit (t *testing.T ){
    ...
    if err.Error() != "open ./keyMock: no such file or directory"{
...
func TestSendMetatransaction (t *testing.T ){
	...
	jsonResponse := relaySignerService.SendMetatransaction(rpcMessage.ID,from,decodeTransaction.To(),decodeTransaction.Data(),new(big.Int).SetUint64(decodeTransaction.Gas()), new(big.Int).SetUint64(decodeTransaction.Nonce()), sig, senderSig)
	...
	if jsonResponse.String() != `{"jsonrpc":"2.0","id":2914410858336929,"result":"0x933dc4274613b4540215dabdaf7661133b147faec010f5cd09dc656506b23294"}` {
